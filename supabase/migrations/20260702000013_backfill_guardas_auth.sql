-- ============================================
-- Backfill: Cria auth.users para todos os guardas existentes sem vinculação
-- Permite que todos os guardas façam login via CPF + senha
-- ============================================

-- 1. Função para provisionar auth.users de todos os guardas sem vinculo
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO auth.users (
        ...
        'authenticated',
        v_email,
        crypt(v_guarda.senha, gen_salt('bf')),
        now(),
        jsonb_build_object(
          'provider', 'email',
