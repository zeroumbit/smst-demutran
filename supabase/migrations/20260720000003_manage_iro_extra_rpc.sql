-- Permite que gestor/administrativo da Guarda edite ou cancele IRO extra
-- por RPC auditado, sem liberar alteracao direta nos registros manuais.
...
AS $$
BEGIN
  IF OLD.adicionado_manual
     AND current_setting('app.iro_manual_rpc', true) IS DISTINCT FROM 'on'
     AND NOT public.is_super_admin() THEN
    RAISE EXCEPTION 'IRO manual nao pode ser alterada ou removida por este perfil.';
  END IF;
...
BEGIN
  ...
  IF NOT public.can_lancar_iro_manual(public.get_guarda_municipal_setor_id()) THEN
