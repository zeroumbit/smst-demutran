-- =====================================================
-- MIGRATION: Relax same-email matching for Guarda managers
-- Fixes self-registration when the manager already has an admin account
-- with the same email but the profile name does not exactly match the
-- functional guarda name.
--
-- Security model kept:
-- 1. the guarda is still validated beforehand by CPF + matricula
-- 2. the reused email must belong to a Guarda-sector admin profile
-- 3. that admin profile must have a functional graduacao_id
-- 4. the graduacao must match the target guarda record
-- =====================================================
...
BEGIN
  ...
  SELECT gm.matricula, gm.graduacao_id
  INTO v_matricula, v_guarda_graduacao_id
