-- Correção RLS e sincronização de listagem de alunos do Jovem Guarda Cidadã

-- 1. Atualizar política RLS de SELECT em jgc_aluno_saude para permitir consulta por perfis que possuem permissão de visualizar alunos (incluindo administrativo)
...
-- 2. Atualizar função jgc_pode_ver_aluno para checar tanto a.turma_id quanto a tabela de junção jgc_aluno_turmas
  ...
  SELECT public.is_super_admin()
    ...
    OR (
      ...
      AND EXISTS (
        ...
        LEFT JOIN public.jgc_aluno_turmas alt ON alt.aluno_id = a.id
        JOIN public.jgc_turma_professores tp ON (tp.turma_id = a.turma_id OR tp.turma_id = alt.turma_id)
...
-- 3. Atualizar jgc_criar_aluno para garantir que se turma_id for informado, também seja inserido em jgc_aluno_turmas
