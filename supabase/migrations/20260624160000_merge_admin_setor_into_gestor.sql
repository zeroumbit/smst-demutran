-- Merge admin_setor into gestor: both papéis had the same semantic meaning.
-- Existing admin_setor records are converted to gestor.
-- RPCs and policies are updated to remove admin_setor references.

-- 1. Convert existing data
UPDATE public.perfis_usuarios
SET papel = 'gestor'::public.papel_usuario
WHERE papel = 'admin_setor'::public.papel_usuario;

-- 2. Update is_admin_of_setor (redefine to remove admin_setor)
...
-- 3. Update contatos RLS policy (remove admin_setor from USING and WITH CHECK)
...
-- 4. Update provision_admin_user: drop all old overloads, recreate with _modulos as jsonb
DROP FUNCTION IF EXISTS public.provision_admin_user(_email text, _password text, _first_name text, _last_name text, _setor_id uuid, _papel public.papel_usuario, _active boolean);
DROP FUNCTION IF EXISTS public.provision_admin_user(_email text, _password text, _first_name text, _last_name text, _setor_id uuid, _papel public.papel_usuario, _modulos text[], _active boolean);
DROP FUNCTION IF EXISTS public.provision_admin_user(_email text, _password text, _first_name text, _last_name text, _setor_id uuid, _papel public.papel_usuario, _active boolean, _modulos text[]);
DROP FUNCTION IF EXISTS public.provision_admin_user(_email text, _password text, _first_name text, _last_name text, _setor_id uuid, _papel public.papel_usuario, _active boolean, _modulos jsonb);
