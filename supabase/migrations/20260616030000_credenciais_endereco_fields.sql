-- Adicionar colunas de endereco separadas na tabela de solicitacoes de credenciais
...
-- Migrar dados existentes: copiar endereco antigo para logradouro como fallback
UPDATE public.demutran_credenciais_solicitacoes
SET
  logradouro = CASE WHEN endereco IS NOT NULL AND endereco <> '' THEN endereco ELSE NULL END
WHERE logradouro IS NULL;

-- Tornar endereco antigo nullable (dados foram migrados)
ALTER TABLE public.demutran_credenciais_solicitacoes
  ALTER COLUMN endereco DROP NOT NULL;

-- Atualizar funcao RPC para aceitar os novos campos de endereco
...
-- Atualizar grants para a nova assinatura da funcao
