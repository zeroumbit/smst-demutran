-- =====================================================
-- Migration: Refactor lancar_iro_extra to use horas instead of date range
--
-- O modal de IROs Extras agora recebe quantidade de horas
-- diretamente (em vez de um intervalo de datas). O RPC
-- cria uma única candidatura na data de início da operação
-- com as horas informadas manualmente.
-- =====================================================

DROP FUNCTION IF EXISTS public.lancar_iro_extra(uuid, uuid, date, date, text);
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Já existe IRO registrada para a data de início da operação (%).', to_char(v_data_ref, 'DD/MM/YYYY');
  END IF;
  ...
  VALUES (
    ...
    'O gestor adicionou ' || round(p_quantidade_horas::numeric, 2) || 'h de IRO referente à operação "' || v_operacao.nome ||
...
GRANT EXECUTE ON FUNCTION public.lancar_iro_extra(uuid, uuid, numeric, text) TO authenticated;
