...
import { Accessibility, ArrowLeft, BarChart3, CheckCircle, Clock, Eye, FileSpreadsheet, FileText, IdCard, Search, SlidersHorizontal, ThumbsDown, ThumbsUp, Users, X, Loader2 } from 'lucide-react';
...
const DemutranRecursos = () => {
  const navigate = useNavigate();
  const { isSuperAdmin, setorId } = useAuth();
  const [items, setItems] = useState<DemutranRecurso[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [editingItem, setEditingItem] = useState<DemutranRecurso | null>(null);
  const [viewingItem, setViewingItem] = useState<DemutranRecurso | null>(null);
  ...
  const [selectedTipo, setSelectedTipo] = useState<'todos' | 'defesa_previa' | 'jari'>('todos');
  ...
  const loadData = async () => {
    ...
    if (error) {
      ...
      setItems((data || []) as DemutranRecurso[]);
    }
    setLoading(false);
  };
  ...
  const handleEdit = (item: DemutranRecurso) => {
    setEditingItem(item);
    setStatus(item.status);
    setObservacao(item.observacao || '');
  };

  const handleView = (item: DemutranRecurso) => {
    setViewingItem(item);
  };
  ...
  }, [items, searchTerm, selectedStatus, selectedTipo, filterYear]);
  ...
  const handleStatusChange = async (id: string, newStatus: DemutranSolicitacaoStatus) => {
    ...
    toast({ title: `Recurso ${statusLabels[newStatus]}` });
    setViewingItem(null);
    loadData();
  };
  ...
  const columns = [
    ...
    { header: 'Nome', accessor: 'nome_completo' as const },
    { header: 'Auto', accessor: 'auto_infracao' as const },
    {
      header: 'Tipo',
      accessor: (item: DemutranRecurso) => tipoLabels[item.tipo] || item.tipo,
    },
    {
      header: 'Status',
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    return count;
  }, [selectedStatus, selectedTipo, filterYear]);
  ...
  const emitidasStats = useMemo(() => {
    ...
    return {
      ...
      defesaPrevia: emitidas.filter((i) => i.tipo === 'defesa_previa').length,
      jari: emitidas.filter((i) => i.tipo === 'jari').length,
    };
  }, [items, filterYear]);

  const baseResourceRows = (items: DemutranRecurso[]) =>
    items.map((item) => ({
      Protocolo: item.protocolo,
      Nome: item.nome_completo,
      Tipo: tipoLabels[item.tipo] || item.tipo,
      Status: statusLabels[item.status],
      Auto_infracao: item.auto_infracao,
      Placa: item.placa || '-',
      CPF: item.cpf,
      ...
      Documento: item.defesa_documento_url,
    }));

  const handleGenerateReport = () => {
    const reportSourceMap: Record<string, { title: string; rows: ReturnType<typeof baseResourceRows> }> = {
      filtros_aplicados: { title: 'Recursos com filtros aplicados', rows: baseResourceRows(filteredItems) },
      todos: { title: 'Todos os recursos', rows: baseResourceRows(filteredItems) },
      pendentes: { title: 'Recursos pendentes', rows: baseResourceRows(filteredItems.filter((item) => item.status === 'pendente')) },
      em_analise: { title: 'Recursos em analise', rows: baseResourceRows(filteredItems.filter((item) => item.status === 'em_analise')) },
      deferidos: { title: 'Recursos deferidos', rows: baseResourceRows(filteredItems.filter((item) => item.status === 'aprovado')) },
      indeferidos: { title: 'Recursos indeferidos', rows: baseResourceRows(filteredItems.filter((item) => item.status === 'rejeitado')) },
      defesa_previa: { title: 'Defesa previa', rows: baseResourceRows(filteredItems.filter((item) => item.tipo === 'defesa_previa')) },
      jari: { title: 'Recursos JARI', rows: baseResourceRows(filteredItems.filter((item) => item.tipo === 'jari')) },
    };
    ...
    const fileName = buildReportFileName('recursos', selected.title);
  ...
  const handleGenerateCustomReport = () => {
    const rows = baseResourceRows(filteredItems);
    if (!rows.length) {
      toast({ title: 'Sem dados', description: 'Nao existem registros para os filtros aplicados.', variant: 'destructive' });
      return;
    }
    const fileName = buildReportFileName('recursos', 'filtros-aplicados');
    if (selectedCustomReportFormat === 'csv') {
      exportReportCsv(fileName, rows);
    } else {
      openPdfPrintReport('Recursos com filtros aplicados', [{ name: 'Filtros aplicados', rows }]);
    }
    setIsCustomReportDialogOpen(false);
  };

  function renderMobileCard(item: DemutranRecurso) {
    return (
      <div
        key={item.id}
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 text-[13px] leading-5 text-slate-500">
              {item.protocolo} &middot; {tipoLabels[item.tipo] || item.tipo}
            </p>
          </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-[13px]">
          <div className="rounded-xl bg-slate-50 px-3 py-2">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">Auto de Infracao</p>
            <p className="mt-0.5 font-semibold text-slate-800">{item.auto_infracao}</p>
          </div>
          <div className="rounded-xl bg-slate-50 px-3 py-2">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">Placa</p>
            <p className="mt-0.5 font-semibold text-slate-800">{item.placa || '-'}</p>
          </div>
          <div className="rounded-xl bg-slate-50 px-3 py-2">
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-1 lg:hidden px-1 py-2">
          ...
          <h1 className="truncate text-lg font-bold tracking-tight text-slate-900">Recursos</h1>
        </div>
          ...
          <div className="space-y-4 px-4 pb-4 pt-5 md:space-y-6 md:px-6 md:pb-5 md:pt-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <h1 className="mt-2 text-xl font-black tracking-[-0.05em] text-white sm:text-2xl md:mt-3 md:text-[32px] md:tracking-[-0.07em] lg:text-[38px]">Recursos</h1>
                <p className="mt-1.5 hidden max-w-xl text-[13px] leading-5 text-white md:block md:mt-2 md:text-[14px] md:leading-6">
                  Receba, analise e acompanhe defesa previa e recursos para JARI.
                </p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <SummaryCard title="Total" value={totalCount} subtitle="Recursos registrados" icon={IdCard} />
              ...
              <SummaryCard title="Deferidos" value={aprovadasCount} subtitle="Recursos concluidos" icon={CheckCircle} />
            </div>
          </div>
        </section>
        ...
        <div>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-foreground">Recursos por ano</h2>
              <p className="text-sm text-muted-foreground">Total de recursos deferidos e concluidos, com separacao por tipo.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                type="button"
                variant={filterYear === 'todos' ? 'default' : 'outline'}
                size="sm"
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-bold text-foreground">Total deferidos</h3>
                <BarChart3 className="h-4 w-4 text-muted-foreground" />
              </div>
              <div className="mt-4">
                <p className="text-3xl font-black tracking-[-0.04em] text-foreground">{emitidasStats.total}</p>
                <p className="mt-1 text-sm text-muted-foreground">Recursos deferidos</p>
              </div>
            </div>

            <div className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-bold text-foreground">Defesa Previa</h3>
                <Users className="h-4 w-4 text-muted-foreground" />
              </div>
              <div className="mt-4">
                <p className="text-3xl font-black tracking-[-0.04em] text-foreground">{emitidasStats.defesaPrevia}</p>
                <p className="mt-1 text-sm text-muted-foreground">Defesas previas deferidas</p>
              </div>
            </div>

            <div className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-bold text-foreground">JARI</h3>
                <Accessibility className="h-4 w-4 text-muted-foreground" />
              </div>
              <div className="mt-4">
                <p className="text-3xl font-black tracking-[-0.04em] text-foreground">{emitidasStats.jari}</p>
                <p className="mt-1 text-sm text-muted-foreground">Recursos JARI deferidos</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    className="flex h-12 w-full appearance-none rounded-[18px] border border-slate-200 bg-slate-50 bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%2394a3b8%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpath%20d%3D%22m6%209%206%206%206-6%22%2F%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[length:20px] bg-[center_right_12px] px-4 pr-11 text-[15px] font-medium text-slate-900"
                    value={selectedTipo}
                    onChange={(event) => setSelectedTipo(event.target.value as 'todos' | 'defesa_previa' | 'jari')}
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="Protocolo, nome ou auto..."
                    />
                  </div>
                </div>
              </div>
        ...
        {loading ? (
          <div className="rounded-[22px] border border-border bg-card p-8 text-center text-muted-foreground">Carregando recursos...</div>
        ) : (
          <>
            ...
            <div className="hidden overflow-hidden rounded-[22px] border border-border bg-card lg:block">
              <DataTable data={filteredItems} columns={columns} onView={handleView} onEdit={handleEdit} emptyMessage="Nenhum recurso registrado" />
            </div>
          </>
        )}
        ...
        <ResponsiveDialog
          ...
          description="Escolha o recorte dos recursos e o formato."
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedReportOption} onValueChange={setSelectedReportOption}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="deferidos">Deferidos</SelectItem>
                  <SelectItem value="indeferidos">Indeferidos</SelectItem>
                  <SelectItem value="defesa_previa">Defesa previa</SelectItem>
        ...
        >
          <div className="space-y-4 py-2">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Tipo</Label>
                <Input value={tipoLabels[editingItem?.tipo || ''] || editingItem?.tipo || ''} disabled />
              </div>
            </div>
            <div className="space-y-2">
...
function ViewDetailsModalContent({
  ...
  const personalInfo = [
    ...
    ['CPF', item.cpf],
    ['Tipo', tipoLabels[item.tipo] || item.tipo],
    ['Auto de Infração', item.auto_infracao],
    ['Placa', item.placa || '-'],
    ['Telefone', item.telefone || '-'],
    ['E-mail', item.email || '-'],
  ];
  ...
  return (
    <div className="space-y-5 py-2">
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Dados do recurso</h4>
        <div className="grid gap-4 md:grid-cols-2">
      ...
      <div>
        <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Documento</h4>
        <DocPreview url={item.defesa_documento_url} label="Documento de defesa" />
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Anote observações sobre este recurso..."
            className="w-full"
          />
...
export default DemutranRecursos;
